...
export default class SR6Model {
  ...
  #performKinematics (axes, scale, parentObject) {
    ...
    if (leftIntersectionPoint && rightIntersectionPoint) {
      ...
      const upperLeftLookAtReversed = this.#toWorldCoordinates(
        upperLeftLink.position.clone().sub(leftIntersectionPointShifted).add(upperLeftLink.position),
        parentObject
      );
      ...
      const upperRightLookAtReversed = this.#toWorldCoordinates(
        upperRightLink.position.clone().sub(rightIntersectionPointShifted).add(upperRightLink.position),
        parentObject
      );
      ...
      const lowerLeftLookAtReversed = this.#toWorldCoordinates(
        lowerLeftLink.position.clone().sub(leftIntersectionPoint).add(lowerLeftLink.position),
        parentObject
      );
      ...
      const lowerRightLookAtReversed = this.#toWorldCoordinates(
        lowerRightLink.position.clone().sub(rightIntersectionPoint).add(lowerRightLink.position),
        parentObject
      );
      ...
      upperLeftFrontBearing.lookAt(this.#toWorldCoordinates(leftIntersectionPointShifted.clone().sub(upperLeftLink.position).add(leftIntersectionPointShifted), parentObject));

      upperRightFrontBearing.up.copy(upVector);
      upperRightFrontBearing.lookAt(this.#toWorldCoordinates(rightIntersectionPointShifted.clone().sub(upperRightLink.position).add(rightIntersectionPointShifted), parentObject));

      lowerLeftFrontBearing.up.copy(upVector);
      lowerLeftFrontBearing.lookAt(this.#toWorldCoordinates(leftIntersectionPoint.clone().sub(lowerLeftLink.position).add(leftIntersectionPoint), parentObject));

      lowerRightFrontBearing.up.copy(upVector);
      lowerRightFrontBearing.lookAt(this.#toWorldCoordinates(rightIntersectionPoint.clone().sub(lowerRightLink.position).add(rightIntersectionPoint), parentObject));
      ...
      if (leftPitcherServoIntersectionPoints) {
        ...
        leftPitcherLink.lookAt(this.#toWorldCoordinates(leftPitcherEndBearingPosition, parentObject));
        ...
        rightPitcherLink.lookAt(this.#toWorldCoordinates(rightPitcherEndBearingPosition, parentObject));
  ...
  #toWorldCoordinates (vector, parentObject) {
    const worldVector = vector.clone().applyAxisAngle(new Vector3(1, 0, 0), this.#orientation);
