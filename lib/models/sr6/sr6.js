  ...
  #performKinematics (axes, scale, parentObject) {
    ...
    if (leftIntersectionPoint && rightIntersectionPoint) {
      ...
      if (leftPitcherServoIntersectionPoints) {
        ...
        const currentLeftPitcherVector = this.#scratchVector2.copy(PITCHER_BEARING_VECTOR).applyAxisAngle(RECEIVER_DIRECTION, -leftPitchServoAngle);
        const correction = correctLeftPitcherVector.angleTo(currentLeftPitcherVector);

        const direction = vectorDirection(correctLeftPitcherVector, currentLeftPitcherVector, X_AXIS);
        ...
        const rightPitcherEndBearingPosition = this.#scratchVector3.copy(leftPitcherEndBearingPosition)
          ...
          if (rightPitcherServoIntersectionPoints) {
            ...
            const currentRightPitcherVector = this.#scratchVector2.copy(PITCHER_BEARING_VECTOR).applyAxisAngle(RECEIVER_DIRECTION, -rightPitchServoAngle);

            const correction = correctRightPitcherVector.angleTo(currentRightPitcherVector);

            const direction = vectorDirection(correctRightPitcherVector, currentRightPitcherVector, X_AXIS);
